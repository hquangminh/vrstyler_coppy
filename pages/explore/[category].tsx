...
const Index = (props: Props) => {
  ...
  const title =
    preTitle +
    (category
      ? category.title + ' 3D Models'
      : category === undefined
      ? 'Explore'
      : (category !== null && props.seo?.title) || '');
...
export const getServerSideProps = checkAuthServerSide(
  async (content) => {
    ...
    if (category !== 'all' && !isUUID(category.split('--')[1]))
      return {
        redirect: {
          destination: urlPage.explore.replace('{category}', 'all'),
          permanent: false,
        },
      };
    ...
    const fetchSeo = commonServices.seoPage('explore', props.language?.langCode);
